distanceFunction_vectors <- function(event1, points1, event2, points2, dmax, tmax, spatial_dist_fun=distHaversine, weight_spatiotemporal = 0.5, spatialORtemporal = 'both'){
	...
	if (length(mainEvent) > 4){
		var1 <- mainEvent[5:length(mainEvent)]
		var2 <- secondEvent[5:length(mainEvent)]
		ln <- length(var1)
